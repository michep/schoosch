...
import 'package:schoosch/old/model/daylessontime_model.dart';
...
class ClassModel {
  ...
  ClassModel.fromMap(this._id, Map<String, dynamic> map) {
    ...
    if (map.containsKey('master') && map['master'] is Map) {
      _master = TeacherPersonModel.fromMap((map['master'] as Map<String, dynamic>)['_id'] as String, map['master'] as Map<String, dynamic>);
    }
    ...
    if (map.containsKey('student') && map['student'] is List) {
      var s = (map['student'] as List).map<StudentPersonModel>((e) {
        var data = e as Map<String, dynamic>;
        return StudentPersonModel.fromMap(data['_id'], data);
      }).toList();
      _students.addAll(s);
  ...
  // Future<TeacherPersonModel?> get master async {
  ...
  // Future<List<StudentPersonModel>> students({bool forceRefresh = false}) async {
