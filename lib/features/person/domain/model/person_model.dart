...
import 'package:schoosch/features/person/domain/model/person_type.dart';
...
class PersonModel {
  ...
  ParentPersonModel? _asParent;
  StudentPersonModel? _asStudent;
  TeacherPersonModel? _asTeacher;
  ObserverPersonModel? _asObserver;
  PersonModel? up;
  ...
  PersonModel.fromMap(this._id, Map<String, dynamic> map, [bool recursive = true]) {
    ...
    map['type'] != null ? types.addAll((map['type'] as List).map((e) => PersonType.parse(e))) : throw 'need type key in people $id';
    if (recursive) {
      if (types.contains(PersonType.admin)) {
        _currentType = PersonType.admin;
      }
      if (types.contains(PersonType.observer)) {
        _asObserver = ObserverPersonModel.fromMap(id, map)..up = this;
        _currentType = PersonType.observer;
      }
      if (types.contains(PersonType.parent)) {
        _asParent = ParentPersonModel.fromMap(id, map)..up = this;
        _currentType = PersonType.parent;
      }
      if (types.contains(PersonType.teacher)) {
        _asTeacher = TeacherPersonModel.fromMap(id, map)..up = this;
        _currentType = PersonType.teacher;
      }
      if (types.contains(PersonType.student)) {
        _asStudent = StudentPersonModel.fromMap(id, map)..up = this;
  ...
  static StudentPersonModel? get currentStudent => currentUser?._asStudent;
  static TeacherPersonModel? get currentTeacher => currentUser?._asTeacher;
  static ParentPersonModel? get currentParent => currentUser?._asParent;
  static ObserverPersonModel? get currentObserver => currentUser?._asObserver;

  StudentPersonModel? get asStudent => _asStudent;
  TeacherPersonModel? get asTeacher => _asTeacher;
  ParentPersonModel? get asParent => _asParent;
  ObserverPersonModel? get asObserver => _asObserver;
...
// STUDENT
class StudentPersonModel extends PersonModel {
  ClassModel? _studentClass;
  ParentPersonModel? parent;
  final Map<String, List<CurriculumModel>> _curriculums = {};

  StudentPersonModel.empty()
      : super.fromMap(null, <String, dynamic>{
          'firstname': '',
          'middlename': '',
          'lastname': '',
          'email': '',
          'type': <String>[PersonType.student.nameString],
        });

  StudentPersonModel.fromMap(String? id, Map<String, dynamic> map) : super.fromMap(id, map, false);
...
// TEACHER
class TeacherPersonModel extends PersonModel {
  ...
  TeacherPersonModel.empty()
      : super.fromMap(null, <String, dynamic>{
          'firstname': '',
          'middlename': '',
          'lastname': '',
          'email': '',
          'type': <String>[PersonType.teacher.nameString],
        });

  TeacherPersonModel.fromMap(String? id, Map<String, dynamic> map) : super.fromMap(id, map, false);
...
// PARENT
class ParentPersonModel extends PersonModel {
  final List<String> studentIds = [];
  final List<StudentPersonModel> _students = [];
  bool _studentsLoaded = false;
  StudentPersonModel? _selectedChild;

  ParentPersonModel.empty()
      : super.fromMap(null, <String, dynamic>{
          'firstname': '',
          'middlename': '',
          'lastname': '',
          'email': '',
          'type': <String>[PersonType.parent.nameString],
          'student_ids': <String>[],
        });

  ParentPersonModel.fromMap(String? id, Map<String, dynamic> map) : super.fromMap(id, map, false) {
  ...
  Future<List<StudentPersonModel>> children({bool forceRefresh = false}) async {
  ...
  Future<StudentPersonModel> get currentChild async {
    return _selectedChild ??= (await children())[0];
  }

  void setChild(StudentPersonModel val) => _selectedChild = val;
...
// OBSERVER
class ObserverPersonModel extends PersonModel {
  final List<String> classIds = [];
  final List<ClassModel> _classes = [];
  bool _classesLoaded = false;

  ObserverPersonModel.empty()
      : super.fromMap(null, <String, dynamic>{
          'firstname': '',
          'middlename': '',
          'lastname': '',
          'email': '',
          'type': <String>[PersonType.observer.nameString],
        });

  ObserverPersonModel.fromMap(String? id, Map<String, dynamic> map) : super.fromMap(id, map, false) {
    map['class_ids'] != null
...
extension PersonTypeList on List<PersonType> {
  bool containsString(String value) {
    return contains(PersonType.parse(value));
  }
